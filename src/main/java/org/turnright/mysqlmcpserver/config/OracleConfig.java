...
public class OracleConfig {
     ...
     */
    @Bean
    @ConditionalOnProperty(prefix = "database.oracle", name = "enabled", havingValue = "true")
    public JdbcDataSourceRegistry oracleDataSourceRegistry() {
        log.info("Initializing Oracle datasources registry");

        String defaultName = properties.getDefaultDatasourceName();
        JdbcDataSourceRegistry registry = new JdbcDataSourceRegistry("Oracle", defaultName);
        ...
        for (DataSourceConfig config : properties.getAllDatasourceConfigs()) {
            ...
            log.info("Registered Oracle datasource: {} (description: {}, readOnly: {})",
    ...
    @Bean
    @ConditionalOnProperty(prefix = "database.oracle", name = "enabled", havingValue = "true")
    public DataSource oracleDataSource(JdbcDataSourceRegistry oracleDataSourceRegistry) {
        return oracleDataSourceRegistry.getDefaultDataSource();
    }
    ...
    private HikariDataSource createHikariDataSource(DataSourceConfig config) {
        ...
        hikariConfig.setPoolName("turnright-oracle-" + config.getName());

        // Oracle specific optimizations
        hikariConfig.addDataSourceProperty("oracle.jdbc.EnableDataLocator", "false");
        ...
        hikariConfig.addDataSourceProperty("oracle.jdbc.TcpNoDelay", "true");
